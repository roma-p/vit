...
def branch_from_origin_branch(
    ...
    with TreeAssetRepo(local_path, package_path, asset_name) as treeAssetRepo:
        ...
        branch_ref = treeAssetRepo.tree_asset.get_branch_current_file(branch_parent)
        if branch_ref is None:
            raise Branch_NotFound_E(asset_name, branch_parent)

        if treeAssetRepo.tree_asset.get_branch_current_file(branch_new):
        ...
        treeAssetRepo.tree_asset.create_new_branch_from_file(
            new_file_path,
            branch_parent,
            branch_new,
            time.time(),
        ...
        treeAssetRepo.ssh_connection.cp(branch_ref, new_file_path)
