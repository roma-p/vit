...
def commit_file(local_path, checkout_file, commit_mess,
    ...
    with TreeAssetRepo(local_path, package_path, asset_name) as treeAssetRepo:
        ...
        treeAssetRepo.tree_asset.update_on_commit(
            path_helpers.localize_path(local_path, checkout_file),
            new_file_path,
            file_track_data["origin_file_name"],
            time.time(), user,
            commit_mess, keep_editable
        )

        treeAssetRepo.ssh_connection.put_auto(checkout_file, new_file_path)
...
def release_editable(local_path, checkout_file):
    ...
    with TreeAssetRepo(
            ...
            file_track_data["asset_name"]) as treeAssetRepo:

        if treeAssetRepo.tree_asset.get_editor(file_track_data["origin_file_name"]) != user:
            raise Asset_NotEditable_E(checkout_file)
        treeAssetRepo.tree_asset.remove_editor(file_track_data["origin_file_name"])
